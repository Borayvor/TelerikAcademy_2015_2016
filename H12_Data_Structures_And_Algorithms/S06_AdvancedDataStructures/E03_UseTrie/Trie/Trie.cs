namespace E03_UseTrie.Trie
{
    ...
    internal class Trie : ITrie
    {
        ...
        private void AddWord(TrieNode trieNode, char[] word)
        {
            ...
            {
                var charWord = Utilities.FirstChar(word);
                TrieNode child;
                trieNode.Children.TryGetValue(charWord, out child);

                if (child == null)
                {
                    child = TrieFactory.GetTrieNode(charWord);
                    trieNode.Children[charWord] = child;
                }

                var charWordRemoved = Utilities.FirstCharRemoved(word);
                this.AddWord(child, charWordRemoved);
            }
        }
